...
const SprintCard = ({ sprint, projectId }) => {
  ...
  return (
    <div className="bg-bg-surface border border-border rounded-lg p-5 hover:shadow-sm transition-shadow">
      ...
      {sprint.goal && (
        <p className="text-sm text-text-secondary mb-4 line-clamp-2">{sprint.goal}</p>
      )}
      ...
      <div className="flex justify-between items-center mt-2 pt-3 border-t border-border">
        <div className="text-xs text-text-muted">
          <span className="font-medium text-text-primary">{completed}</span> / {total} pts
          {sprint.story_count !== undefined && (
            <span className="ml-3">{sprint.story_count} stories</span>
          )}
        </div>
